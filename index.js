...
import { 
    getFirestore, doc, getDoc, getDocs, collection, updateDoc, increment 
...
window.loadFullGallery = async () => {
    ...
    try {
        // Firestore: Fetch 'media' subcollection
...
async function processSearch(imageUrl) {
    ...
    try {
        ...
        // Firestore: Fetch all media docs to compare faces
        const mediaCol = collection(db, 'clients', eventId, 'media');
        const snap = await getDocs(mediaCol);
        ...
        if (!snap.empty) {
            snap.forEach(doc => {
                ...
                // Firestore Note: 'descriptors' is now an array of objects: [{values: [...]}, {values: [...]}]
                if (photo.descriptors && photo.descriptors.some(d => {
                    // Handle the object wrapper for descriptor values
...
window.submitDeletionRequest = async () => {
    ...
    try {
        ...
        // Firestore: Update the main client document
