...
// Explicitly allow Multiple Login Access (Persist session locally)
// This ensures that refreshing the page or opening a new tab keeps the user logged in
...
// Reset Login UI state so multiple attempts/users can try seamlessly
function resetLoginUI() {
    elements.phoneStep1.style.display = 'block';
    elements.phoneStep2.style.display = 'none';
    elements.loginOtp.value = '';
    // Don't clear phone number if user just wants to retry, 
    // but if you want complete clear, uncomment next line:
...
async function ensureUserProfile(user) {
    ...
    const updates = {
        ...
        // Only update location if not present, to prevent overwriting other sessions' location data unintentionally
...
elements.sendOtpBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // This handles "multiple login access" - allowing re-sending/new login on same number
...
async function handleLogout() {
    try {
        ...
        resetLoginUI(); // Reset UI on logout so next login is fresh
    } catch (error) {
        ...
        if (view) {
            if (view === 'products') loadProducts();
                ...
                if (!appState.currentUser) {
                    ...
                    resetLoginUI(); // Reset state before showing
                    elements.loginModal.classList.add('open');
                    // Ensure recaptcha renders
