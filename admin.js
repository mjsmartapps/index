...
async function processAndUploadFiles(files, clientId) {
    ...
    const uploadSingle = async (file, index) => {
        ...
        try {
            ...
            if (file.type.startsWith('image/') && typeof faceapi !== 'undefined') {
                ...
                try {
                    ...
                    if(!faceapi.nets.ssdMobilenetv1.params) {
                        ...
                        // FIX: WRAP ARRAY IN OBJECT FOR FIRESTORE
                        // Firestore cannot store [[1,2], [3,4]]. It must be [{values:[1,2]}, {values:[3,4]}]
            ...
            if(msg.includes("Failed to fetch")) msg = "R2/Network Error";
            if(msg.includes("Nested arrays")) msg = "Data Format Error (Fixed)";
